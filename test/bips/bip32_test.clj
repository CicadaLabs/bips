...
(deftest derive-master-code-ckd-priv-neutered-test-vector-1
  (let [seed "000102030405060708090a0b0c0d0e0f"
        ...
        base58-encoded-private-key (serialize :mainnet :private 0 0 0
        ...
        base58-encoded-child-private-key (serialize :mainnet :private 1 fingerprint (math/expt 2 31)
                                                    (:chain-code child)
                                                    (:private-key child))
        neutered (N {:private-key (:private-key child)
                     :chain-code (:chain-code child)
                     :index 1})
        base58-encoded-neutered-child-public-key (serialize :mainnet :public 1 fingerprint (math/expt 2 31)
                                                            (:chain-code neutered)
                                                            (:public-key neutered))]
...
(deftest derive-master-code-ckd-priv-pub-test-vector-2
  (let [seed "fffcf9f6f3f0edeae7e4e1dedbd8d5d2cfccc9c6c3c0bdbab7b4b1aeaba8a5a29f9c999693908d8a8784817e7b7875726f6c696663605d5a5754514e4b484542"
        ...
        child (CKDpriv {:private-key (:private-key master-node)
                        ...
                        :index 0})
        fingerprint (key-fingerprint (:public-key master-node))
        base58-encoded-child-private-key (serialize :mainnet :private 1 fingerprint 0
                                                    (:chain-code child)
                                                    (:private-key child))
        childp (CKDpub {:public-key (:public-key master-node)
                        :chain-code (:chain-code master-node)
                        :index 0})
        base58-encoded-child-public-key (serialize :mainnet :public 1 fingerprint 0
                                                   (:chain-code childp)
                                                   (:public-key childp))]
